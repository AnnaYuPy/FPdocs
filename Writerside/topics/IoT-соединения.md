...
1. Настройка привязки клиентского HTTP соединения
...
На первом этапе сервер проверит, является ли полученный от клиента сертификат доверенным (соответствует ли издатель клиентского сертификата издателю сертификата сервера, сертификат сервера указывается в настройках системы). При этом Common Name (CN) у пользовательского сертификата не должно совпадать с CN серверного сертификата. Если на этом этапе возникают ошибки при попытке соединения с сервером, для тестирования соединения можно воспользоваться утилитой командной строки curl:
...
2. Настройка привязки серверного HTTP соединения
...
### 1.2. Настройка привязки клиентского HTTP соединения
